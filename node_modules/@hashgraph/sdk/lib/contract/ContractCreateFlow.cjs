...
class ContractCreateFlow {
  ...
  async executeWithSigner(signer) {
    ...
    const fileCreateTransaction = await new _FileCreateTransaction.default().setKeys(key != null ? [key] : []).setContents(this._bytecode.subarray(0, Math.min(this._bytecode.length, 2048))).freezeWithSigner(signer);
